...
class CustomMediaGallery extends Component {
  constructor() {
    super();
    this.state = {
      dimensions: {
  ...
  onDrop(files) {
    ...
    //Set the path prefix for Amazon 
    const pathPrefix = getPathPrefix( this.props.productId );
    const metaContext = {productId: product._id};

    //Upload media using Slingshot Directive
    let productImageUploader = new Slingshot.Upload( "productImage", metaContext);
...
registerComponent({
  name: "customMediaGallery",
  component: CustomMediaGallery
});
